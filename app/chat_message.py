...
# chatbot_service = ChatbotService(model_path="app/model/")  # Moved to lazy loading
...
def get_chatbot_service():
    ...
    if not hasattr(get_chatbot_service, '_instance'):
        get_chatbot_service._instance = ChatbotService(model_path="app/model/")
...
def analyze_conversation(session_id):
    ...
    try:
        bert_result = get_chatbot_service().predict(conversation_text)
    ...
    for msg in messages:
        clean_text = get_chatbot_service().preprocess_text(msg.content)
        phrases = get_chatbot_service().extract_phrases(clean_text)
